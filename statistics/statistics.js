fetch('http://127.0.0.1:8986/stats').then(r => r.json()).then(json => {
    ...
    document.querySelector('.stats-gamezip div:last-child').textContent    = json.formatTotals[0].count;
    document.querySelector('.stats-legacy div:last-child').textContent     = json.formatTotals[1].count;
